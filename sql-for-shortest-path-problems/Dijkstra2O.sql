...
SELECT /*+ GATHER_PLAN_STATISTICS TWOWAY */ LPad (node, 1 + 2 * (lev - 1), '.') node, lev, path, cost, lp
  FROM paths_ranked
  WHERE rnk_t = 1
  ORDER BY line_no
/
EXECUTE Utils.Write_Plan (p_sql_marker => 'TWOWAY');
...
SELECT LPad (node, 1 + 2* (lev - 1), '.') node, lev, path, cost, rnk_t, rnk, lp
  ...
  ORDER BY line_no
/
PROMPT BPF Solution - two-way, all intermediate, breadth first
WITH paths (node, path, cost, rnk, lev) AS (
SELECT a.dst, a.src || ',' || a.dst, a.distance, 1, 1
  FROM arcs a
WHERE a.src = :SRC
...
PROMPT BPF Solution - two-way from J
WITH paths (node, path, cost, rnk, lev) AS (
SELECT a.dst, a.src || ',' || a.dst, a.distance, 1, 1
  FROM arcs a
WHERE a.src = :SRC
