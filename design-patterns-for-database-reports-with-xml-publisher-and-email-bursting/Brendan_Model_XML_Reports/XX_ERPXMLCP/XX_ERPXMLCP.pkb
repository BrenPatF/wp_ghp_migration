CREATE OR REPLACE PACKAGE BODY XX_ERPXMLCP AS
/**************************************************************************************************

Author:		Brendan Furey, 20 September 2014
Description:	Package body for package for XML Publisher report: XX Example XML CP (Printed),
                XX_ERPXMLCP, as described in:
...
FUNCTION HdrFilter (    p_col_hdr_1             VARCHAR2,
                        p_col_hdr_2             VARCHAR2,
                        p_lin_count             PLS_INTEGER,
                        p_lin_2_count           PLS_INTEGER) RETURN BOOLEAN IS
BEGIN

  Write_Log (RPad (p_col_hdr_1, 20) || ' ' || RPad (p_col_hdr_2, 20) || ' ' || LPad (p_lin_count, 10) || ' ' || LPad (p_lin_2_count, 10), FALSE);
...
BEGIN
  ...
  Write_Log (' ', FALSE);
  Write_Log (RPad ('App', 20) || ' ' || RPad ('Program', 20) || ' ' || LPad ('# params', 10) || ' ' || LPad ('# groups', 10), FALSE);
  Write_Log (RPad ('=', 20, '=') || ' ' || RPad ('=', 20, '=') || ' ' || LPad ('=', 10, '=') || ' ' || LPad ('=', 10, '='), FALSE);
  RETURN TRUE;
